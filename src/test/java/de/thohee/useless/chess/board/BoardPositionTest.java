...
public class BoardPositionTest {
	...
	public void testGetPossibleCastlings() {
		...
		boardPosition.set(Coordinate.get(4, 0), new Piece(Colour.White, Figure.King));
		boardPosition.addCastlingPiece(boardPosition.get(Coordinate.get(4, 0)));
		// rook is missing
		boardPosition.analyze();
		assertTrue(boardPosition.getPossibleCastlings(Colour.White).isEmpty());
		boardPosition.set(Coordinate.get(7, 0), new Piece(Colour.White, Figure.Rook));
		// rook is not registered as castling piece
		boardPosition.analyze();
		assertTrue(boardPosition.getPossibleCastlings(Colour.White).isEmpty());
		boardPosition.addCastlingPiece(boardPosition.get(Coordinate.get(7, 0)));
		// king side castling
		boardPosition.analyze();
		...
		boardPosition.set(Coordinate.get(5, 0), new Piece(Colour.White, Figure.Bishop));
		// blocked by bishop
		boardPosition.analyze();
		assertTrue(boardPosition.getPossibleCastlings(Colour.White).isEmpty());
		boardPosition.set(Coordinate.get(0, 0), new Piece(Colour.White, Figure.Knight));
		boardPosition.addCastlingPiece(boardPosition.get(Coordinate.get(0, 0)));
	...
	public void testGetPosssibleKingSideCastlingAfterQueenSideRookMoved() {
		...
		for (Colour colour : Arrays.asList(Colour.White)) {
			...
			boardPosition = boardPosition.performMove(new Move(colour, Figure.Rook, Coordinate.get(0, row),
					Coordinate.get(0, (row + 2) % 7), Capture.None));
			boardPosition.analyze();
	...
	public void testGetPossibleCastlingsThroughChess() {
		...
		boardPosition.set(Coordinate.get(3, 4), new Piece(colour.opposite(), Figure.Queen));
		boardPosition.analyze();
		List<Move> possibleCastlings = boardPosition.getPossibleCastlings(colour);
	...
	public void testGetPossibleMoves() throws IllegalMoveFormatException {
		...
		final Coordinate c04 = Coordinate.get(0, 4);
		for (Colour colour : Colour.values()) {
			BoardPosition boardPosition = new BoardPosition();
			// top of c04 is blocked by own pieces
			boardPosition.set(Coordinate.get(0, 5), new Piece(colour, Figure.Pawn)); // blocks the rook, queen, king and
																						// white pawn
			boardPosition.set(Coordinate.get(1, 5), new Piece(colour, Figure.Pawn)); // blocks the bishop, queen and
																						// king
			boardPosition.set(Coordinate.get(1, 6), new Piece(colour, Figure.Pawn)); // blocks the knight
			// below c04 is 'blocked' by opponent's pieces
			Colour opposite = colour.opposite();
			boardPosition.set(Coordinate.get(0, 3), new Piece(opposite, Figure.Pawn)); // beaten by rook, queen, king
																						// and black pawn
			boardPosition.set(Coordinate.get(2, 2), new Piece(opposite, Figure.Pawn)); // beaten by bishop and queen,
																						// threatens black king
			boardPosition.set(Coordinate.get(1, 2), new Piece(opposite, Figure.Pawn)); // beaten by knight
			for (Figure figure : Figure.values()) {
				...
				if (colour.equals(Colour.Black)) {
					...
					boardPositionWithPiece.set(Coordinate.get(3, 1), new Piece(Colour.White, Figure.Pawn));
					boardPositionWithPiece = boardPositionWithPiece.performMove(new Move(Colour.White, Figure.Pawn,
							Coordinate.get(3, 1), Coordinate.get(3, 2), Capture.None));
				}
				...
				case Pawn:
					...
					break;
				case Knight:
					expectedMoves.add(new Move(colour, figure, c04, Coordinate.get(2, 3), Capture.None));
					expectedMoves.add(new Move(colour, figure, c04, Coordinate.get(2, 5), Capture.None));
					expectedMoves.add(new Move(colour, figure, c04, Coordinate.get(1, 2), Capture.Regular));
					break;
				case Bishop:
					expectedMoves.add(new Move(colour, figure, c04, Coordinate.get(1, 3), Capture.None));
					expectedMoves.add(new Move(colour, figure, c04, Coordinate.get(2, 2), Capture.Regular));
					break;
				case Rook:
					for (int c = 1; c < 8; ++c) {
						expectedMoves.add(new Move(colour, figure, c04, Coordinate.get(c, 4), Capture.None));
					}
					expectedMoves.add(new Move(colour, figure, c04, Coordinate.get(0, 3), Capture.Regular));
					break;
				case Queen:
					for (int c = 1; c < 8; ++c) {
						expectedMoves.add(new Move(colour, figure, c04, Coordinate.get(c, 4), Capture.None));
				...
				case King:
					if (colour.equals(Colour.White)) {
						expectedMoves.add(new Move(colour, figure, c04, Coordinate.get(1, 4), Capture.None));
						expectedMoves.add(new Move(colour, figure, c04, Coordinate.get(0, 3), Capture.Regular));
						expectedMoves.add(new Move(colour, figure, c04, Coordinate.get(1, 3), Capture.None));
					}
					break;
		...
		for (Colour colour : Colour.values()) {
			...
			Coordinate lastPosition = Coordinate.get(0, 3);
			Move lastMove = new Move(lastPiece.getColour(), lastPiece.getFigure(), lastPosition,
					Coordinate.get(lastPosition.getColumn(), lastPosition.getRow() - direction), Capture.None);
			// one step or capture
			{
				BoardPosition boardPosition = new BoardPosition();
				Coordinate startPosition = Coordinate.get(column, nextRow);
				...
				expectedMoves.add(new Move(colour, Figure.Pawn, startPosition,
						Coordinate.get(column, nextRow + direction), Capture.None));
				assertEquals(expectedMoves, new HashSet<Move>(getPossibleMoves(boardPosition, startPosition)));
				// regular captures
				for (int dc : Arrays.asList(-1, 1)) {
					Coordinate captureTarget = Coordinate.get(column + dc, nextRow + direction);
			...
			{
				BoardPosition boardPosition = new BoardPosition();
				Coordinate startPosition = Coordinate.get(column, initialRow);
				boardPosition.set(startPosition, pawn);
				boardPosition.set(lastPosition, lastPiece);
				boardPosition.setLastMove(new MoveMock(lastMove.getColour().opposite()));
				boardPosition = boardPosition.performMove(lastMove);
				Set<Move> expectedMoves = new HashSet<>();
				expectedMoves.add(new Move(colour, Figure.Pawn, startPosition,
						Coordinate.get(column, initialRow + direction), Capture.None));
				expectedMoves.add(new Move(colour, Figure.Pawn, startPosition,
						Coordinate.get(column, initialRow + 2 * direction), Capture.None));
				assertEquals(expectedMoves, new HashSet<Move>(getPossibleMoves(boardPosition, startPosition)));
			}
			...
			{
				BoardPosition boardPosition = new BoardPosition();
				Coordinate startPosition = Coordinate.get(column, otherSideRow);
				boardPosition.set(startPosition, pawn);
				int otherColumn = startPosition.getColumn() + 1;
				Coordinate otherTargetPosition = Coordinate.get(otherColumn, startPosition.getRow());
				Coordinate otherStartPosition = Coordinate.get(otherColumn,
						otherTargetPosition.getRow() + 2 * direction);
				Set<Move> expectedMoves = new HashSet<>();
				expectedMoves.add(new Move(colour, Figure.Pawn, startPosition,
						Coordinate.get(column, otherSideRow + direction), Capture.None));
				expectedMoves.add(new Move(colour, Figure.Pawn, startPosition,
						Coordinate.get(otherColumn, otherSideRow + direction), Capture.EnPassant));
			...
			{
				BoardPosition boardPosition = new BoardPosition();
				Coordinate startPosition = Coordinate.get(column, beforeLastRow);
				boardPosition.set(startPosition, pawn);
				boardPosition.set(lastPosition, lastPiece);
				boardPosition.setLastMove(new MoveMock(lastMove.getColour().opposite()));
				boardPosition = boardPosition.performMove(lastMove);
				...
				Coordinate targetPosition = Coordinate.get(column, beforeLastRow + direction);
	...
	public void testPerformMove() {
		...
		for (Colour colour : Colour.values()) {
			for (Figure figure : Figure.values()) {
				Piece piece = new Piece(colour, figure);
				BoardPosition boardPosition = new BoardPosition();
				boardPosition.set(Coordinate.get(4, 4), piece);
				// first move for white or unrelated last move by white before
				if (colour.equals(Colour.Black)) {
					boardPosition.set(Coordinate.get(7, 5), new Piece(Colour.White, Figure.Pawn));
					boardPosition = boardPosition.performMove(new Move(Colour.White, Figure.Pawn, Coordinate.get(7, 5),
							Coordinate.get(7, 6), Capture.None));
				}
	...
	public void testPerformCastling() throws IllegalMoveFormatException {
		...
		preparedBoardPosition.set(Coordinate.get(4, 0), whiteKing);
		preparedBoardPosition.set(Coordinate.get(0, 0), whiteRook1);
		preparedBoardPosition.set(Coordinate.get(7, 0), whiteRook2);
		preparedBoardPosition.set(Coordinate.get(4, 7), blackKing);
		preparedBoardPosition.set(Coordinate.get(0, 7), blackRook1);
		preparedBoardPosition.set(Coordinate.get(7, 7), blackRook2);
			...
			for (Colour colour : Colour.values()) { // 1. white 2. black
				...
				if (colour.equals(Colour.Black)) {
					...
					boardPosition = boardPosition.performMove(new Move(colour.opposite(), Figure.King,
							Coordinate.get(4, oppositeRow), Coordinate.get(4 + direction, oppositeRow), Capture.None));
				}
				Move castlingMove = new Move(colour, castling);
				boardPosition = boardPosition.performMove(castlingMove);
				assertEquals(6, boardPosition.getNumberOfPieces());
				assertEquals(kings.get(colour.ordinal()), boardPosition.get(Coordinate.get(4 + 2 * direction, row)));
				assertEquals(rooks.get(colour.ordinal() * 2 + castling.ordinal()),
						boardPosition.get(Coordinate.get(4 + direction, row)));
				assertNull(boardPosition.get(Coordinate.get(rookColumn, row)));
				assertNull(boardPosition.get(Coordinate.get(4, row)));
			}
		}
	}
	...
	public void testPerfomCaptureEnPassant() throws IllegalMoveFormatException {
		...
		assertTrue(boardPosition.getPossibleMoves().contains(move));
		boardPosition = boardPosition.performMove(move);
		assertEquals(1, boardPosition.getNumberOfPieces());
	...
	public void testIsCheckmate() {
		{
			BoardPosition boardPosition = new BoardPosition();
			boardPosition.set(Coordinate.get(0, 0), new Piece(Colour.White, Figure.King));
			boardPosition.set(Coordinate.get(0, 2), new Piece(Colour.Black, Figure.Queen));
			assertTrue(boardPosition.isCheck());
		...
		}
		{
			...
			boardPosition.set(Coordinate.get(0, 0), new Piece(Colour.White, Figure.King));
			boardPosition.set(Coordinate.get(0, 2), new Piece(Colour.Black, Figure.Queen));
			boardPosition.set(Coordinate.get(2, 0), new Piece(Colour.Black, Figure.Rook));
...

		{
			BoardPosition boardPosition = new BoardPosition();
			boardPosition.set(Coordinate.get(0, 0), new Piece(Colour.White, Figure.King));
			boardPosition.set(Coordinate.get(1, 0), new Piece(Colour.White, Figure.Pawn));
			boardPosition.set(Coordinate.get(0, 1), new Piece(Colour.White, Figure.Pawn));
			boardPosition.set(Coordinate.get(2, 2), new Piece(Colour.Black, Figure.Queen));
			assertFalse(boardPosition.isCheckmate());
		}
	...
	public void testCheckMate2() throws Exception {
		...
		GameReport gameReport = games.iterator().next();
		BoardPosition boardPosition = BoardPosition.getInitialPosition();
	...
	private Set<Move> expectedInitiallyPossibleMoves(Colour colour) {
		...
		for (int c = 0; c < 8; ++c) {
			expectedMoves.add(new Move(colour, Figure.Pawn, Coordinate.get(c, pawnRow),
					Coordinate.get(c, pawnRow + direction), Capture.None));
			expectedMoves.add(new Move(colour, Figure.Pawn, Coordinate.get(c, pawnRow),
					Coordinate.get(c, pawnRow + 2 * direction), Capture.None));
		}
		expectedMoves.add(new Move(colour, Figure.Knight, Coordinate.get(1, pawnRow - direction),
				Coordinate.get(0, pawnRow + direction), Capture.None));
		expectedMoves.add(new Move(colour, Figure.Knight, Coordinate.get(1, pawnRow - direction),
				Coordinate.get(2, pawnRow + direction), Capture.None));
		expectedMoves.add(new Move(colour, Figure.Knight, Coordinate.get(6, pawnRow - direction),
				Coordinate.get(5, pawnRow + direction), Capture.None));
		expectedMoves.add(new Move(colour, Figure.Knight, Coordinate.get(6, pawnRow - direction),
				Coordinate.get(7, pawnRow + direction), Capture.None));
	...
	public void testGetInitialPosition() throws IllegalMoveFormatException {
		BoardPosition initialPosition = BoardPosition.getInitialPosition();
		assertEquals(32, initialPosition.getNumberOfPieces());
		initialPosition.analyze();
		assertTrue(initialPosition.getPossibleCastlings(Colour.White).isEmpty());
	...
	public void testGetDepth() {
		...
		for (int column = 0; column < 8; ++column) {
			for (Colour colour : Colour.values()) {
				...
				int direction = colour.equals(Colour.White) ? 1 : -1;
				Move move = new Move(colour, Figure.Pawn, Coordinate.get(column, row),
						Coordinate.get(column, row + direction), Capture.None);
				assertTrue(boardPosition.getPossibleMoves().contains(move));
				boardPosition = boardPosition.performMove(move);
				depth++;
			}
	...
	public void testIsDraw() throws IllegalMoveFormatException {
		...
		for (int m = 1; n < 100; ++m) {
			for (Colour colour : Arrays.asList(Colour.White, Colour.Black)) {
				...
				Coordinate nextPosition = Coordinate.get(c, r);
